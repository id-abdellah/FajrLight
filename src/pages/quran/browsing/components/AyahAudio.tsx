...
export default function AyahListen({ audioURL }: Props) {
  const [audioState, setAudioState] = useState<'pause' | 'play' | 'loading'>(
    'pause',
  );
  ...
  return (
    <button
      className="bg-primary hover:bg-hover_primary"
      onClick={() => audioControle()}
    >
      {audioState == 'pause' ? (
