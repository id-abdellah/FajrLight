...
export default function PrayerTimesPage() {
    ...
    const { data: responseData, isLoading, isError } = useQuery({
        queryFn: () => API.dayPrayerTimes(currentDate, settings.prayerTimes.location.latitude, settings.prayerTimes.location.longitude),
        queryKey: ["currentDayPrayerTimes", settings.prayerTimes.location.latitude, settings.prayerTimes.location.longitude],
    ...
    return (
        <section>
            ...
            <Countdown currentDate={currentDate} setNextPrayerID={setNextPrayerID} nextPrayerID={nextPrayerID} currentPrayersTimings={prayerTimesData.timings} />

            <MonthTimingsCalendar prayerTimesData={prayerTimesData} />

            <div className="w-4/5 mx-auto mt-4 md:w-[65%]">
                {Object.entries(prayerTimesData.timings).map(([prayerID, prayerTime], i) => {
            ...
            <div className="flex flex-col items-center mt-6 select-none">
                <span className="text-secondary/90 font-bold">إِنَّ الصَّلَاةَ كَانَتْ عَلَى الْمُؤْمِنِينَ كِتَابًا مَّوْقُوتًا</span>
                <span className="text-xs mt-1 text-on_surface">النساء 103</span>
            </div>
